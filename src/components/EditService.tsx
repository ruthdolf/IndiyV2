...
import { db, doc, getDoc, updateDoc, handleFirestoreError, OperationType, storage, ref, uploadBytes, uploadString, uploadBytesResumable, getDownloadURL, firebaseConfig } from '../firebase';
import { useAuth } from '../contexts/AuthContext';
import { Service, ServiceStatus } from '../types';
import { Card } from './ui';
import { useNavigate, useParams } from 'react-router-dom';
import { ArrowLeft, Plus, X, Loader2, Image as ImageIcon, Save, Upload, Video, CheckCircle2, AlertCircle, Info } from 'lucide-react';
...
export const EditService: React.FC = () => {
  ...
  const [originalSellerInfo, setOriginalSellerInfo] = useState<{ id: string, name: string, avatar?: string } | null>(null);
  ...
  useEffect(() => {
    const fetchService = async () => {
      ...
      try {
        ...
        handleFirestoreError(err, OperationType.GET, `services/${id}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  const onDropImages = useCallback(async (acceptedFiles: File[]) => {
    ...
    setUploading(true);
    setError('');
    addLog(`Processing ${acceptedFiles.length} updated image(s)...`, 'info');
    ...
    try {
      ...
      addLog(`Updated ${urls.length} images.`, 'success');
    } catch (err: any) {
  ...
  const onDropVideo = useCallback(async (acceptedFiles: File[]) => {
    ...
    try {
      ...
      addLog(`Connecting to Storage (Edit)...`, 'network');
      
      const storageRef = ref(storage, storagePath);
      
      const snapshot = await uploadBytes(storageRef, file);

      addLog(`Binary stream complete (Edit).`, 'success');
      setProgress(prev => ({ ...prev, [file.name]: 90 }));

      const downloadURL = await getDownloadURL(snapshot.ref);
      setVideoUrl(downloadURL);
      addLog('Video successfully linked (Edit).', 'success');
      setProgress(prev => ({ ...prev, [file.name]: 100 }));
    } catch (err: any) {
      if (err.code === 'storage/unauthorized') {
        addLog(`PERMISSION DENIED (Edit): Sync your Storage Rules in the Firebase Console.`, 'error');
        setError(`Security Error: Storage Rules in Console are blocking this update.`);
      } else {
  ...
  const onDropDownloadableFile = useCallback(async (acceptedFiles: File[]) => {
    ...
    try {
      ...
      const fileSizeMB = (file.size / (1024 * 1024)).toFixed(2);
      addLog(`File: ${sanitizedName} (${fileSizeMB}MB)`, 'info');
      addLog(`Connecting as (Edit): ${user.email} (${user.uid})`, 'network');
      ...
      addLog('File successfully hosted (Edit).', 'success');
      setProgress(prev => ({ ...prev, [file.name]: 100 }));
    } catch (err: any) {
      if (err.code === 'storage/unauthorized') {
        addLog(`PERMISSION DENIED (Edit): Sync your Storage Rules in the Firebase Console.`, 'error');
        setError(`Security Error: Storage Rules are blocking this file.`);
      } else {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // Only sync seller info if the person editing is the actual owner
      // This prevents Admins from accidentally "becoming" the seller when they edit a gig
      if (user.uid === originalSellerInfo?.id) {
        updateData.sellerName = user.displayName || undefined;
        updateData.sellerAvatar = user.photoURL || undefined;
      }
      ...
      if (isSizeError) {
        setError('Total image data is too large. Please remove 1-2 images to fit within database limits.');
      } else if (isPermissionError) {
        setError('Update rejected by server. This usually means a field violates a rule (e.g. title too long). Please check your edits.');
      } else {
  ...
  if (loading) return <div className="p-20 text-center">Loading service details...</div>;
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="mb-10">
        <h1 className="text-3xl font-display font-bold mb-2">Edit Service</h1>
        <p className="text-text-muted">Update your service details and keep your listing fresh.</p>
      </div>
          ...
          <div className="space-y-8">
            <Card className="p-8 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-4">
                  <label className="block text-sm font-bold uppercase tracking-widest text-text-muted">Status</label>
                  <select 
                    value={status}
                  ...
                  {!downloadUrl ? (
                    ...
                    >
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-text-muted mt-1">Files up to 100MB</p>
                      </div>
                    </div>
                  ) : (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-xs font-bold text-text-primary truncate max-w-[200px]">
                            {downloadFileName || 'File linked/uploaded'}
                          </p>
